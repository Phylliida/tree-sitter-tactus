...
void *tree_sitter_tactus_external_scanner_create() { return ts_calloc(1, sizeof(Scanner)); }

void tree_sitter_tactus_external_scanner_destroy(void *payload) { ts_free((Scanner *)payload); }

unsigned tree_sitter_tactus_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    buffer[0] = (char)scanner->opening_hash_count;
    return 1;
}

void tree_sitter_tactus_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
...
bool tree_sitter_tactus_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
